...
def test_format_message_does_not_include_full_body_by_default(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_format_message_includes_body_when_opt_in(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    module, _tokens = load_hook(tmp_path, monkeypatch)
    monkeypatch.setenv("CODEX_NOTIFY_INCLUDE_BODY", "1")
    body = "A" * 1200
    message = module.format_message(
        {
            "type": "agent-turn-complete",
